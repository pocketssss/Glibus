...
local function normalize_angle(n)
    return n - doublepi * floor((n + pi) * inv_doublepi)
end
...
local function sincos(n)
    n = n - doublepi * floor((n + pi) * inv_doublepi)
    local x2 = n * n
...
function math.rad(n) return n * rad_coeff end
function math.deg(n) return n * deg_coeff end

local rand_state = 1

function math.SharedRandomSeed(seed)
    rand_state = (floor(abs(seed)) % 2147483646) + 1
end
